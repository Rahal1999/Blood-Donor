...
export class AfterCareComponent implements OnInit {
  ...
  loadPreviousDataFromLocalStorage() {
    ...
    const afterCareDataKey = `afterCare_${loggedInUser.email}`;
  ...
  checkForInactivity() {
    ...
    if (!savedData[todayKey]) {
      ...
      this.notificationService.addNotification(
        `You have not submitted your after-care data for today (${new Date().toDateString()}). Please update it!`
      );
    }
  }
  ...
  submitAfterCare() {
	...
	this.notificationService.setUserType('donor');
	this.notificationService.addNotification(
	  'Your after-care checklist has been successfully submitted. Thank you for your cooperation!'
	);
